...
foreach ($output as $i => $process_line) {
  if ($first_line) {
    ...
    if ($time_in_seconds > TIME_THRESHOLD) {
      $markup = kill_pid_and_remove_job($this_pid, $this_payload, $conn, $time_in_seconds);
      if ($markup) {
        $run_output[] = $markup;
      }
    }
...
if (count($run_output) > 0) {
  echo "The CRON job on " . $server_name . " /opt/islandora_cron/kill_pid_after_time.php is killing some OCR jobs because they exceeded the theshold of " . TIME_THRESHOLD . " seconds.\n\n";
...
function kill_pid_and_remove_job($this_pid, $this_payload, $conn, $time_in_seconds) {
  $ret_markup = '';
